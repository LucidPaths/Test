...
| BASELINE_RATE | `app/src/stores/savingsStore.ts` (exported const) | `getBlendedRate()` fallback in same file | Single source — used only for game simulation (2% Tagesgeld) |
| PROJECTION_RATE | `app/src/constants/gameBalances.ts` (exported const) | `OnboardingView.tsx` (pitch calc + display), `CompoundCurve.tsx` (chart default) | Single source — pitch and chart always agree on default rate (4%) |
